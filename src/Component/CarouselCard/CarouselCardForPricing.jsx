...
const CarouselCardForPricing = ({ 
  itemWidth, 
  itemHeight, 
  items = [], 
  scalingRatio = 1.4,  // Default scaling ratio for the centered item
  margin = 0 // Default margin between items
}) => {
      ...
      {getVisibleItems().map((item, index) => (
        <div
          ...
          style={{
            ...
            transform: index === 1 ? `scale(${scalingRatio})` : 'scale(1)', // Centered item scales based on the scalingRatio prop
          }}
        >
          {item.content}
        </div>
...
export default CarouselCardForPricing;
